...
function cross_act_ntf(self, ntf_id, ...)
    ...
    if prop then
        ...
        if prop.Chat1 then
            Rpc:chat({pid=-1,gid=_G.GateSid}, 0, 0, 0, "system", "", prop.Chat1, arg[1])
        end
        if conf.SendMail then
            player_t.send_system_to_all(conf.SendMail, arg[3] or {}, arg[1] or {}, arg[2])
        end
    end
end

function post_npc_change(self, propid, map_id, tag)
    cross_mng.npc_change(self.pid, propid, map_id, tag)
end
...
function cross_act_st_req(self, param)
    cross_mng.cross_act_st_req(self.pid)
end

function upload_gs_info(self, gs_info)
    cross_mng.upload_gs_info(gs_info)
end
