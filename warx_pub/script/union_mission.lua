...
function update(uid,ply,exp)--刷新军团定时任务品质
    ...
    if _d[uid].class  < #_q and _d[uid].state == TASK_STATUS.TASK_STATUS_CAN_ACCEPT  then
        if not ply:do_dec_res(resmng.DEF_RES_GOLD, get_gold(ply.pid,uid) , VALUE_CHANGE_REASON.UNION_MISSION) then
            return
        end
        ...
        else _d[uid].log[cur]={id=cur,pid=ply.pid,exp=exp} end
        gPendingSave.union_mission[uid] = _d[uid]
    end
end
...
function add(uid)--领取军团任务奖励
    local d = _d[uid]
    if not d then WARN("没有任务:"..uid) return end
        ...
        if cc then
            ...
            if u then
                ...
                for _,v  in pairs(_members or {}) do
                    if  can_date(v._union.tm_mission) or  (v._union.cur_item < d.cur_item) then
                        union_item.add(v,cc.Item,UNION_ITEM.TASK)
                        v._union.tm_mission = gTime
                        v._union.cur_item = d.cur_item
                        gPendingSave.union_member[v.pid] = v._union
                    end
                end
                u:add_log(resmng.UNION_EVENT.MISSION,resmng.UNION_MODE.OK,{ propid=d.propid,mode=d.cur_item })
            end
        else WARN("没有礼物:"..id) end
    end
end
