...
function accept_hero_road_chapter(self, chapter_id, line_id)
	local prop_chapter = resmng.get_conf("prop_hero_road_chapter", chapter_id)
	if prop_chapter == nil then
		return
	end

	local task_array = prop_chapter.TaskList[line_id]
	...
	if chapter_data ~= nil and chapter_data[line_id] ~= nil then
		return
	end
	...
	local line_data = chapter_data.task_line[line_id]
...
function get_hero_road_task_award(self, chapter_id, line_id, task_id)
	if self.hero_road_cur_chapter ~= chapter_id then
		return
	end

	local chapter_data = self.hero_road_chapter[chapter_id]
	if chapter_data == nil or chapter_data.task_line == nil or chapter_data.task_line[line_id] == nil then
		return
	end
