...
function random_gacha(self, group_id)
	...
	for i = sid, eid, 1 do
		...
		if cur_p >= p then
			...
			if award == nil then
				return prop_tmp.BonusPolicy,prop_tmp.Bonus
			else
				return award[1],award[2]
			end
		end
	end
end
...
function do_yinbi_one(self, msg_send)
    ...
    local bonus_policy, bonus = self:random_gacha(group_id)
    local res = self:add_bonus_not_notify(bonus_policy, bonus, VALUE_CHANGE_REASON.REASON_GACHA_AWARD_YINBI_ONE)
    --local res = self:add_bonus(bonus_policy, bonus, VALUE_CHANGE_REASON.REASON_GACHA_AWARD_YINBI_ONE)
    if res == true then
    	table.insert(msg_send.award, bonus[1])
