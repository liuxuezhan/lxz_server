...
delete <breakpointnum>           -- Delete a breakpoint
disable                          -- Disable debugger
list                             -- List all breakpoints
<statement>                      -- execute a statement in the current context
helpall                          -- Print all debug commands

The following commands are ONLY available when script is stopped:
...
setb =    [[
setb [file line]    -- set a breakpoint to file/line|, line 0 means 'any'
...
local function debugger_loop(ev, vars, file, line, idx_watch)
  ...
  while true do
    ...
    elseif command == "listb" or command == "list" then
      --{{{  list breakpoints
    ...
    elseif command == "run" or command == "c" then
    ...
    elseif command == "over" or command == "n" then
    ...
    elseif command == "trace" or command == "bt" then
      --{{{  dump a stack trace
...
_G.coroutine.create = function(f)
  ...
  if hook then
    ...
    thread = cocreate(function(...)
                        ...
                        step_level [thread] = 0
                        --debug.sethook(thread_hook,mask.."c",count)
...
io.read = function( mode )
    ...
    gCoroDgb = co
    local inputs =  coroutine.yield("ok")
    gCoroDgb = nil
    return inputs
end
